...
// Fetches an activity's laps. Structured watch workouts record one lap per
// rep and recovery; a run with no lap presses comes back as a single lap.
// Only used when the activity detail doesn't already carry `laps`.
async function fetchActivityLaps(activityId, accessToken) {
  const resp = await fetch(`${STRAVA_API_BASE}/activities/${activityId}/laps`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });

  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`Strava laps fetch failed: ${resp.status} ${text}`);
  }
...
//   3. Laps from a structured watch workout (see classifyFromLaps) —
//      reps separated by jogs average out far slower than the reps.
//   4. Otherwise, average pace against personal training zones:
...
//   5. A run that still comes out 'easy' is upgraded to 'tempo' if its
//      mile splits contain a tempo block (see hasTempoBlock) — a tempo
//      session with a warm-up and cool-down averages out far slower than
//      the tempo miles themselves.
const PACE_SPEED_MAX = 380; // sec/mi — faster than this = speed
const PACE_TEMPO_MAX = 410; // sec/mi — faster than this = tempo

// Split-based tempo detection: at least MIN_TEMPO_MILES of back-to-back
// splits each at TEMPO_SPLIT_MAX or faster. 7:15/mi sits just outside the
// plan's slowest prescribed tempo (7:10/mi), so on-plan tempos qualify.
const TEMPO_SPLIT_MAX = 435; // sec/mi
const MIN_TEMPO_MILES = 2;

// True if the splits contain a contiguous stretch of at least
// MIN_TEMPO_MILES where every split is at TEMPO_SPLIT_MAX or faster.
// Uses each split's own distance, so a trailing partial mile counts only
// for the distance it actually covers.
function hasTempoBlock(splits) {
  if (!Array.isArray(splits)) return false;
  let blockMiles = 0;
  for (let i = 0; i < splits.length; i++) {
    const splitMiles = splits[i].mile - (i === 0 ? 0 : splits[i - 1].mile);
    if (splits[i].pace_sec != null && splits[i].pace_sec <= TEMPO_SPLIT_MAX) {
      blockMiles += splitMiles;
      if (blockMiles >= MIN_TEMPO_MILES) return true;
    } else {
      blockMiles = 0;
    }
  }
...
// Lap-based workout detection, for runs recorded as a structured workout
// (one lap per rep/recovery). Speed: at least MIN_SPEED_LAPS laps of
// MIN_SPEED_LAP_METERS or more, each at LAP_SPEED_MAX or faster — 6:30/mi
// sits between the plan's slowest rep pace (6:20) and fastest tempo (6:40).
// Tempo: laps at TEMPO_SPLIT_MAX or faster adding up to MIN_TEMPO_MILES,
// counted across recovery laps since reps are split up by jogs.
const LAP_SPEED_MAX = 390; // sec/mi
const MIN_SPEED_LAPS = 2;
const MIN_SPEED_LAP_METERS = 300;

// Returns 'speed', 'tempo', or null (no laps, a single lap, or no workout
// pattern found — leave it to the other rules).
function classifyFromLaps(laps) {
  if (!Array.isArray(laps) || laps.length < 2) return null;

  let speedLaps = 0;
  let tempoMiles = 0;
  for (const lap of laps) {
    const seconds = lap.moving_time || lap.elapsed_time;
    const miles = (lap.distance || 0) / METERS_PER_MILE;
    if (!seconds || miles <= 0) continue;
    const paceSec = seconds / miles;
    if (paceSec <= LAP_SPEED_MAX && lap.distance >= MIN_SPEED_LAP_METERS) speedLaps++;
    if (paceSec <= TEMPO_SPLIT_MAX) tempoMiles += miles;
  }

  if (speedLaps >= MIN_SPEED_LAPS) return 'speed';
  if (tempoMiles >= MIN_TEMPO_MILES) return 'tempo';
  return null;
}

// `splits` (output of computeMileSplits) and `laps` (Strava laps) are
// optional; without them, a run is classified from its average pace alone.
function mapActivityToRun(activity, splits, laps) {
  ...
  if (activity.workout_type === 1) {
    ...
    type = 'long';
  } else if (classifyFromLaps(laps)) {
    type = classifyFromLaps(laps);
  ...
  if (type === 'easy' && hasTempoBlock(splits)) {
    type = 'tempo';
  }
